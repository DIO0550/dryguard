...
`src/lib.rs` を直した。
'
expect_result "宣言が 2 行あれば落ちる" 1 "宣言が 2 行あります" '差分規模: 2 ファイル / +3 -1

1 巡目の差分規模: 1 ファイル / +1 -0
'
expect_result "装飾が付いていても読む" 0 "差分規模の宣言は実際の差分と一致しています" '| 項目 | 値 |
|---|---|
| **差分規模** | **2 ファイル / +3 -1** |
'
# 数を持たない地の文だけでは宣言と見なさない（見なすと、数の突き合わせを
# 飛ばしたまま通ってしまう）。
expect_result "差分規模という語だけの行は宣言にしない" 1 "差分規模の宣言がありません" '差分規模は本文の冒頭に書く。'
