...
| `rule-staleness` | AGENTS.md「実装を変えたら、rules/ 配下の記述も同じ PR で出し直す」（`rules/*.md` が指す実装の詳細（語彙の定義・辿る関数名など）が、同じ PR の後半の修正で古くなったまま残る。`comment-mismatch` はコード内コメントが対象、`unverified-claim` は記録・PR 本文・Issue コメント・コミットメッセージ・レビュー返信・通知が対象で、どちらも `rules/*.md` を射程に入れていない。`分類: なし` から昇格。pr-155 指摘 3・pr-186 指摘 12 が同じ形で、後者は前者を「2 件目」と自己申告している） |
