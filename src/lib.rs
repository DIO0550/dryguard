...
//!
//! `lsp` は Phase 2 の土台として、サーバの起動・JSON-RPC の往復・握手・
//! ワークスペースの根の受け渡し・候補ペアのファイルの開閉・hover の問い合わせを持つ。
//! `semantics` は hover が返した綴りを正規化し、型シグネチャが単一化できるかまでを出す。
//!
//! **その型シグネチャはまだ判定に効いていない。** シグナルとして `classification` へ
//! 渡すのはこの次で、今 `Signals` に入っているのは `syntax` が採るものだけ。
//!
