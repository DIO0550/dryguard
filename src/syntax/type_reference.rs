...
/// シグネチャの一部になる。**自分の名前を持つチャンクでは hover が関数自身の型を返す**ので、
/// 外側の注釈の綴りはそのままでは現れない。ただし**注釈を省いた位置には、そこから
/// 推論された型名が出る**（`const f: (x: Input) => void = function g(x) {…}` の `Input`。
/// Issue #188）ので、「外側の注釈が一切効かない」ではない。
///
/// **Why not（名前付きチャンクでも代入先の注釈を集める）**: その型名が出るのは**注釈を
/// 省いた位置**だけで、その出現は綴りで引かない（`semantics::type_signature` の
/// `NormalizedSignature::spelled_positions`）ので、集めても答えは動かない。実コーパス
/// （rxjs 7.8.1 の `src`、尋ねた 517 チャンク）で測ると `Normalized` は 314 件のまま
/// 1 件も増えず、代わりに理由が「注釈を書くと辿れる」へ変わって**注釈を書いてある
/// 利用者に嘘の案内**が戻る（Issue #202）。
///
