//! 正規化したトークンと、その集合の重なり。
//!
//! Phase 0 の素朴実装。チャンク化と import 収集が構文木に移った後も、ここだけは
//! 文字を前から見てトークンに切っている（`docs/dryguard-plan.md`「Phase 1」）。
//!
//! テンプレートリテラルの `${}` の中と正規表現リテラルは、この切り方では読めない。
//! どちらも 1 つの [`Token::Text`] / 記号の並びとして潰れる。ここを詰めるのは
//! **AST 正規化（識別子→プレースホルダ / リテラル→型タグ）を入れる回**の仕事で、
//! 同じ回にトークンの語彙と類似度の測り方がまとめて変わる。
