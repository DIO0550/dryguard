...
pub enum CallerDomainOverlap {
    ...
    /// どちらかのチャンクが名前を持たず、尋ねる位置を決められなかった。
    NoName,
    /// プロジェクトの印を持たないファイルがあり、参照元が揃うか確かめられない。
    ///
    /// **印の下に無いファイルでは、根をどれだけ広げても参照元は揃わない。** サーバは
    /// 開かせたファイルとその import 先だけでプロジェクトを組み立て、呼び出し元はその
    /// 向きの逆にあるため（`lsp::ServerCommand::project_markers`）。
    ///
    ...
    /// 印はあるが、サーバはそのファイルをその印のプロジェクトの一員として扱っていない。
    ///
    /// 印のファイルは `files` / `include` / `exclude` で範囲を絞れる。範囲から外れた
    /// ファイルには、**祖先に印があっても**サーバがその場で組み立てたプロジェクトが
    /// 割り当たり、参照元は開かせていない呼び出し元を取りこぼす。
    ///
    /// **`ProjectUnrooted` と分ける。** どちらも参照元が揃わないが、
    /// **利用者が直す先が違う**（印を置く / 印の範囲を直す）
    /// (`rules/architecture.md`「理由は落とさない」)。
    OutsideProject {
        /// 探した印の名前。範囲を直す相手がどのファイルかを出すのに要る。
        markers: Vec<String>,
    },
    /// 印はあるが、サーバに所属を尋ねる手立てが無く、参照元が揃うか確かめられない。
    ///
    /// **印の下にあることは一員であることを意味しない。** 印のファイルは範囲を絞れるので、
    /// 尋ねられないサーバでは範囲内かどうかを言えない。
    ///
