...
**その Issue で決めたことをやり終えたら閉じる。残ったタスクは新しい Issue にする。**

- **「直さない」も完了。** 測った結果・却下した案・その理由をコメントに残したうえで閉じる
- **残タスクが 1 つでもあれば、閉じる前に新しい Issue を作ってリンクする。**
  「あとでやる」を元の Issue のコメントに残さない
- 新しい側には**何をスコープ外にしたか**を書く（「タスクの分割」と同じ形）

**Why**: 開いたままの Issue は「まだ何か残っている」と読める。判断が済んでいるのに開いていると、
次に見た人が**どこまで決まったのかをコメントの並びから読み直す**ことになる。

**Why not（残タスクがあるあいだ開けておく）**: 1 つの Issue が複数の未完了を抱えると、
**どれが終わってどれが残っているかが構造に出ない**。コメントを最後まで読んで突き合わせることになり、
Issue の一覧（開いているか閉じているか）からは読めない。
...
- **書いた時点では正しかった主張が、後から入った変更で古くなることがある。** 出し直すのは
  ...
  更新されずずれていた）
- **出し直す時点を「push の後」に置かない。push するたびに、その push で古くなった記述を
  本文の頭から読み直す。** 再発した回はどれも**規約を読んだうえで** push の後に気づいている
  （pr-185 指摘 3 / pr-191 指摘 6 がどちらも「規約はあり、push の前ではなく後に気づいた」と
  書いている。pr-194 指摘 6 は「本文の更新を push と同じ手順に入れる」と自分で宣言したのに、
  同じ PR の指摘 8 で再発している）
- **差分規模だけは層 1 が無条件に突き合わせる。** 本文の**行の頭**に
  `差分規模: <ファイル数> ファイル / +<追加行> -<削除行>` を置く
  （箇条書きの `- ` と太字の `**` は付いていてよい。2 行以上書くなら数を揃える。
  `.github/workflows/pr-body.yml` / `harness/ci/pr-body-diff.sh`）。
  「**ツールで落とせるものは規約の文に留めない**」（下の「強制力の序列」）の適用で、
  **リポジトリから出し直せる数**はここが落とす。**覆うのは差分規模だけ**——
  引用した文言・数えた件数・主張の条件は本文にしか無いので、上の 2 項目が持つ
...
（pr-101 / pr-140 指摘 10 が、この射程の違いを取り違えたことを記録している）。

**分類の語彙は射程ではなく、書いた時点で正しかったかで割る。** `unverified-claim` は
**書く時点で確かめていなかった**形（射程はどちらでも同じタグ）、`claim-staleness` は
**書いた時点では正しかったのに、後続の変更に追従せず古くなった**形
（`harness/records/TEMPLATE.md`）。1 件の指摘が両方に当たることは無いので、
`count.sh` の数が 2 つに割れない。

## 実装を変えたら、rules/ 配下の記述も同じ PR で出し直す

ここに入るのも、**2 回以上出た指摘**だけ。

- **`rules/*.md` が指す実装の詳細（語彙の定義・辿る関数名など）は、同じ PR の後半の
  修正で変わることがある。修正を入れたら、その回のうちに `rules/` 側の記述も読み直して
  出し直す。** 書きっぱなしにすると、**規約が既に無効になった振る舞いを説明したまま**残り、
  次に読む人がそこへ戻しうる
  （pr-155 指摘 3: `rules/naming.md` の `project root` の定義を「共通の祖先から上へ
  探した結果」と書いた後、同じ PR の指摘 1 の修正でファイルごとに探す形へ変えたのに、
  語彙表を直していなかった。pr-186 指摘 12: `rules/architecture.md` が指す helper 名
  `constructed_class_reference_of` が、同じ PR の指摘 10 の修正で型変数の制約まで
  広げたことで存在しないものになっていた。pr-186 自身が「`pr-155.md` 指摘 3 と同じ形で、
  これが 2 件目」と自己申告している。pr-189 指摘 1 は、`pr-186.md` を書く過程で一度
  「1 件目」と書き、既存の記録を数え直さずに書いた誤りとしてレビューで訂正されたことを
  記録している）

**Why not（`comment-mismatch` に含める）**: `comment-mismatch` は rules/coding.md
「コメントは doc と Why / Why not に絞る」に紐づき、対象は**コード内コメント**。
`rules/*.md` の記述はコードコメントではなく、対象を広げると
「コード内コメントの食い違い」と「規約文書の食い違い」という**直し方の違う 2 種類**が
1 つの分類に畳まれる。

**Why not（`unverified-claim` に含める）**: `unverified-claim` は
AGENTS.md「記録・PR 本文に数値や主張を書く前に」に紐づき、対象は**記録・PR 本文・
Issue コメント・コミットメッセージ・レビュー返信・通知**に限られる。`rules/*.md` は
そのどれでもない。**書いた時点では正しくても後から古くなる**性質は
`claim-staleness` と共通だが、**古くなる相手が違う**（`rules/*.md` は実装が変わると古びる。
記録・PR 本文はその回に入れた修正に追従できずに古くなる）ので、直し方も
「実装を読み直す」と「その push の差分を読み直す」に分かれる。
...
（「実装を始める前に」「記録・PR 本文に数値や主張を書く前に」「実装を変えたら、
rules/ 配下の記述も同じ PR で出し直す」）に置く。`plan` の項目が pr-77 以来この扱いで、
`unverified-claim` も同じ理由（dryguard の構造規約ではなく、報告の書き方の話）で
ここに続けた。`rule-staleness` も同じ理由（dryguard の構造規約そのものではなく、
`rules/*.md` という文書を実装とどう同期させ続けるかという運用の話）でここに続けた。
