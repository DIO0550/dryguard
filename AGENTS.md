...
（pr-110 が記録自身の内訳・分類・回数について 16 件、続けて pr-129 指摘 9・pr-132 指摘
35 / 36 / 39 / 40・pr-135 指摘 4・pr-138 指摘 5・pr-140 指摘 7 / 8 / 11 / 12 / 13 と
記録されている）。
...
- **記録・PR 本文・Issue コメント・コミットメッセージ・レビュー返信・通知に書く数値や主張は、
  ...
  （pr-110: 記録内の内訳・分類・回数が実データと 16 箇所食い違った。pr-140 指摘 11 / 12:
...
この形のうち `harness/records/` 配下（pr-110 など）を既に射程に入れている。上の項目は
その規約が射程に入れていない PR 本文・Issue コメント・コミットメッセージ・レビュー返信・
通知まで対象を広げたもので、2 つは射程が異なる別の規約として並立する（pr-101 / pr-140
指摘 10 が、この射程の違いを取り違えたことを記録している）。分類の語彙上はどちらの射程も
`unverified-claim` としてまとめる（`harness/records/TEMPLATE.md`）。
