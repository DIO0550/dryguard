...
（pr-110 が 16 件・pr-140 が「同じ形」と明記した 4 件を含め、pr-101 / pr-105 / pr-129 /
pr-132 / pr-135 / pr-138 / pr-140 と続けて記録されている）。

- **PR 本文・Issue コメント・コミットメッセージ・レビュー返信・通知に書く数値や主張は、
...
`harness/records/README.md`「推測で埋めない」は近いが、**記録そのものを書くときの決まり**で、
PR 本文・Issue コメント・レビュー返信・通知は射程に入っていない（pr-101 / pr-140 指摘 10 が
この取り違えそのものを記録している）。射程が違う規約を「既存ルールあり」と読み違えないよう、
ここへ別項目として置く。
