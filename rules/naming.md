...
| `untraced type name` | 比較に残る綴りに現れたのに、**その出現を辿った記録が無い**型名。尋ねる位置を作れなかったもので、**ソースに書かれていない**（推論された値の型・推論された引数の型の型名）か、書かれているのに**集める場所の一覧から漏れた**か、**どの出現が書かれていないかを言えない**かのどれか。**束縛された型変数は入らない** |
| `value type` | チャンクが受け渡しする値の型。関数・getter では戻り値の型、setter では引数の型。**`annotatable position` の 1 つ**で、省くと TypeScript が推論する |
| `type annotation` | ソースが**その位置**に型注釈を書いたか（書かれている / 省かれている）。省かれていれば hover はそこに推論した型を綴るので、**その出現は尋ねていない** |
...
同じ語になり、`--explain` を付けると根拠そのものが増えるように読める（増えるのは
**出す範囲**で、判定に効いた根拠は既定でも全部出ている）。
...
**共用体の並びは、相手の並べ替えを試して落とす。** 型変数の番号は出現順に振るので、
書かれた並びのままでは**相手をまたいで番号の付き方が変わり**、同じ型が別の構造のまま残る
（`<V, A>(acc: V | A) => A` と `<V, A>(acc: A | V) => A`）。相手の並べ替えを試して
**正規化した形の最小を採る**ことで、番号の付き方を書かれた並びから切り離す
（`syntax::type_structure` の `searched_normal_form`）。

**ただし組み合わせには上限があり、超えたら書かれた並びのまま比べる。** 数えるのは
**番号を割り当てる相手だけ**なので、リテラルやキーワードだけの共用体は上限を食わない。
倒れる向きは偽陰性で、上限を超えるかは構造から決まるので、**同じ型の 2 つは必ず揃って
超えるか超えないか**になる（片方だけ探索されて食い違うことは起きない）。
...
尋ねていないのは**対象のコードか dryguard の側**の話。
1 語で呼ぶと、`--explain` を読んだ利用者が直す先を選べなくなる
（`rules/coding.md`「エラー型は原因ごとにバリアントを分ける」と同じ形）。

**`untraced type name` の 3 通りを混ぜない。** `type annotation` を省いたのと、
辿った記録が無いことしか言えないのと、**どの出現が書かれていないかを言えない**のとでは、
**言い切れることが違う**。1 つ目は注釈が無いことを構文木から確かめてあるので
「注釈を書くと辿れる」と出してよいが、2 つ目は**書かれているのに集め損ねただけかもしれない**
（集める場所の一覧は TypeScript の文法が持つ形の数だけ増え続ける）。1 語で呼ぶと、
**注釈を書いてある利用者に「書かれていない」と出す**ことになる
（`semantics::type_signature` の `UntracedReason`）。

**3 つ目は、ソースの引数の数と hover の綴りの引数の数が揃わなかった形。** 出どころを
出現ごとに持てるのは**添字で対応が取れるあいだだけ**なので、揃わなければ引数の位置すべてを
省かれた扱いへ倒す（偽陰性側）。**どの引数が省いたのかを言えない**ので、1 つ目の文を
そのまま出すと同じ嘘の案内になる。

**1 つ目を引数の位置で出すのは、辿った記録がある綴りに限る。** 引数の型は**チャンクの外に
書かれた注釈**から来ることがある（名前付き関数式が代入先の注釈から受け取る形）ので、
記録が 1 つも無いときは 2 つ目を先に出す（`rules/architecture.md`
「どこまでを「取れなかった」に数えるか」）。

**語は増やさない。** どれも `type annotation` と `traced type name` で言えるので、
新しい語ではなく `untraced type name` の内訳として置く（この節の最後）。

**`site-dependent spelling` を `untraced type name` と混ぜない。** どちらも比較に残る綴りに
現れて「測れない」へ倒れるが、**利用者が次にすることが違う**。尋ねていない型名は
**型名ではある**ので注釈を書けば位置ができうるが、場所に依存する綴りは**型名ですらない**ので、
注釈を書くだけでは位置ができない。1 語で呼ぶと、注釈を書いた利用者が同じ「測れない」で
戻ってくる（`rules/coding.md`「エラー型は原因ごとにバリアントを分ける」と同じ形）。

**ただし `site-dependent spelling` の側に共通の直し先は無い。** `typeof localValue` なら
その型に名前を付けられるが、**`this` 型は囲むクラスの外へ出せない**（型エイリアスに移せず、
...
**`untraced type name` は型名 1 つではなく出現 1 つを指す。** 同じ綴りが 1 つのシグネチャに
2 回現れ、片方だけが書かれていることがある（`function build(a: Amount): Amount` の戻り値の
注釈を省いた形）。型名を単位にすると、**書かれていない出現が書かれた出現の記録に乗る**
（`rules/architecture.md`「どこまでを「取れなかった」に数えるか」）。

**`value type` を「戻り値の型」と呼ばない。** 関数と getter では戻り値だが、setter では
**引数の型**が hover の綴りになる。1 語で「戻り値」と呼ぶと、**戻り値を注釈できない
setter がまとめて「注釈が無い」側へ落ちる**（コンストラクタも同じ形だが、あちらは
囲むクラスの宣言が綴りを書いているので「書かれている」側）。

**`value type annotation` と `parameter annotation` を混ぜない。** どちらも
`annotatable position` の `type annotation` だが、**値の型はチャンクにつき 1 つ、引数は
引数ごとに答えが違う**。1 つに畳むと、**既定値つきの引数が 1 つあるだけで、注釈を書いてある
引数と戻り値までまとめて測れない側へ落ちる**（Issue #200 の案 1。実コーパスで
`Normalized` を 4 件失い、塞げたのは 0 件）。

**引数だけは添字で出現を突き合わせられる。** `Callable::parameters` もソースの引数リストも
順序を持つので、**どの出現が書かれていないか**を位置で言える。戻り値・型引数にこの鍵は無い
（`rules/architecture.md`「どこまでを「取れなかった」に数えるか」）。

**`value type annotation` を `type reference` と混ぜない。** どちらもソースに書かれた注釈の
話だが、前者は**その位置に注釈が在るか**（型名を 1 つも持たない `: void` でも「書かれている」）、
後者は**その注釈に書かれた型名と、尋ねる位置**。1 語で呼ぶと、注釈は在るのに型名が
1 つも無いシグネチャを「尋ねていない」側へ倒すことになる。
...
**`callee` と `reference` を混ぜない。** どちらも 1 点を指して尋ねた結果のファイルの位置だが、
...
**呼び出し元集合と呼び出し先集合を突き合わせる呼び出しが書けてしまう**
（`rules/coding.md`「不正な状態を型で表現できなくする」）。中身を共有しても、
`jaccard` が `&Self` を取る限り取り違えはコンパイルで落ちる。**中身のほうを両側に並べない** —
`jaccard` には「件数では重み付けしない」という判定の材料の作り方が埋まっており、
2 箇所に置くと片方だけ直したときに呼び出し元と呼び出し先で数え方が食い違う。

**`callee` を `import` と混ぜない。** どちらも依存先を指すが、`import` は**ファイル単位**の
宣言で、`callee` は**そのチャンクが実際に呼んでいるもの**。同じファイルに複数ドメインの
`import` があると、`import` だけではチャンクごとの依存先を分けられない（Issue #30）。

**`call hierarchy item` を `callee` と混ぜない。** 前者は**尋ねるための起点**、後者は
**尋ねた答え**。1 語で呼ぶと、起点が 2 つ返ったこと（どれを起点にするか決められない）と、
呼び出し先が 2 件返ったこと（答えが 2 件）が同じ形に見える。
