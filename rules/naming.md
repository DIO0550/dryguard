...
| `untraced type name` | 比較に残る綴りに現れたのに、**その出現を辿った記録が無い**型名。尋ねる位置を作れなかったもので、**ソースに書かれていない**（推論された戻り値の型名）か、書かれているのに**集める場所の一覧から漏れた**かのどちらか。**束縛された型変数は入らない** |
...
**`untraced type name` の 2 通りを混ぜない。** `value type annotation` を省いたのと、
辿った記録が無いことしか言えないのとでは、**言い切れることが違う**。前者は注釈が無いことを
構文木から確かめてあるので「注釈を書くと辿れる」と出してよいが、後者は**書かれているのに
集め損ねただけかもしれない**（集める場所の一覧は TypeScript の文法が持つ形の数だけ
増え続ける）。1 語で呼ぶと、**注釈を書いてある利用者に「書かれていない」と出す**ことになる
（`semantics::type_signature` の `UntracedReason`）。

**語は増やさない。** どちらも `value type annotation` と `traced type name` で言えるので、
