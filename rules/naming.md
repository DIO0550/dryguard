...
**ただし共用体の並びが落ちきらないことがある。** 型変数の番号は出現順に振り、並べ替えはその後なので、
**相手をまたいで番号の付き方が変わる**と同じ型が別の構造のまま残る（`<V, A>(acc: V | A) => A` と
`<V, A>(acc: A | V) => A`。型変数が 1 つだけなら変わらないので落ちる）。
倒れる向きは偽陰性で、閉じる案は Issue #195。
