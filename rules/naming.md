...
**ただし共用体の並びが落ちきるのは、相手に型変数が絡まないときだけ。** 型変数の番号は
出現順に振り、並べ替えはその後なので、**共用体の相手をまたぐと書かれた並びで番号が変わる**
（`syntax::type_structure` の `Callable::normalized`）。倒れる向きは偽陰性で、閉じる案は Issue #195。
